  ...
  template: `
    ...
    >
      ...
      <div class="p-4 grid grid-cols-1 md:grid-cols-3 gap-4 items-start">
        <div
          class="col-span-1 rounded-2xl border p-5 bg-gradient-to-b from-accent-50 to-white"
        >
          <div class="text-sm text-slate-600">Today</div>
          <div class="mt-1 text-4xl font-black tracking-tight text-slate-900">
            {{ state().totalMl
            }}<span class="text-base font-semibold ml-1">ml</span>
          </div>
          ...
          <div class="mt-4 flex items-center gap-2">
            <button (click)="add(250)" class="px-3 py-2 rounded-xl border">
              +250ml
            </button>
            <button (click)="add(500)" class="px-3 py-2 rounded-xl border">
              +500ml
            </button>
            <button (click)="add(1000)" class="px-3 py-2 rounded-xl border">
              +1L
            </button>
          </div>
        </div>
        ...
        <div class="col-span-2 rounded-2xl border p-5 space-y-3">
          ...
          <div class="pt-2 border-t">
            ...
            <div class="text-xs text-slate-500 mt-1">
              Requires notifications permission. Schedules local alerts at
...
export class WaterIntakeComponent {
  ...
  private loadToday(): WaterState {
    const key = todayKey();
    const saved = load<WaterState>("brobot_water", {
      dateKey: key,
      totalMl: 0,
      targetMl: 2500,
      log: [],
    });
    if (saved.dateKey !== key)
      return {
        dateKey: key,
        totalMl: 0,
        targetMl: saved.targetMl || 2500,
        log: [],
      };
    return saved;
  }
  ...
  progress() {
    const p = Math.min(
      100,
      Math.round(
        (this.state().totalMl / Math.max(500, this.state().targetMl)) * 100,
      ),
    );
    return p;
  }
  ...
  add(n?: number) {
    ...
    const next: WaterState = {
      ...s,
      totalMl: s.totalMl + amt,
      log: [{ amount: amt, ts: Date.now() }, ...s.log],
    };
    this.state.set(next);
    this.persist();
  }
  ...
  async toggleReminders() {
    ...
    if (this.remind) {
      ...
      if (perm !== "granted") {
        this.remind = false;
        localStorage.setItem("brobot_water_remind", "0");
        return;
      }
      const now = new Date();
      for (const hh of ids) {
