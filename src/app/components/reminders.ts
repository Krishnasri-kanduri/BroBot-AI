    ...
    >
      <div class="px-5 py-4 border-b border-slate-200">
        ...
        <p class="text-xs text-slate-500">
          Daily routines and one-off occasions
        </p>
      </div>

      <div
        *ngIf="perm !== 'granted'"
        class="relative z-10 pointer-events-auto mx-4 mt-4 mb-0 p-3 rounded-xl bg-amber-50 border border-amber-200 text-amber-800 text-sm flex items-center justify-between"
        aria-live="polite"
      >
        <div>
          Notifications are off. Enable to get popup alerts at reminder time.
        </div>
        <button
          type="button"
      ...
      <form
        (submit)="add($event)"
        class="p-4 grid grid-cols-1 md:grid-cols-5 gap-3"
      >
        <input
          [(ngModel)]="title"
          name="title"
          ...
          class="md:col-span-2 px-3 py-2 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-brand-500"
        />
        <select
          [(ngModel)]="type"
          name="type"
          class="px-3 py-2 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-brand-500"
        >
        ...
        <div *ngIf="type === 'occasion'" class="flex gap-2">
          <select
            [(ngModel)]="occMonth"
            name="occMonth"
            class="px-3 py-2 rounded-xl border focus:outline-none focus:ring-2 focus:ring-brand-500"
          >
          ...
          <input
            ...
            min="1"
            max="31"
            class="w-24 px-3 py-2 rounded-xl border focus:outline-none focus:ring-2 focus:ring-brand-500"
            placeholder="Day"
          />
        </div>
        <button
          type="submit"
          class="px-4 py-2 rounded-xl bg-brand-600 text-white hover:bg-brand-700"
        >
          Add
        </button>
      </form>
      ...
      <div class="px-4 pb-4 space-y-3">
        <div
          *ngFor="let r of reminders()"
          class="flex items-center justify-between p-3 rounded-xl border bg-white"
        >
          <div>
            <div
              class="font-medium text-slate-800"
              [class.line-through]="r.done"
            >
              {{ r.title }}
            </div>
            <div class="text-xs text-slate-500">
              <ng-container *ngIf="r.type === 'daily'"
                >Daily at {{ r.time }}</ng-container
              >
              <ng-container *ngIf="r.type === 'occasion'"
                >On {{ formatOccasion(r.date) }}</ng-container
              >
            </div>
          </div>
        ...
        <div
          *ngIf="reminders().length === 0"
          class="text-sm text-slate-500 p-4 text-center"
        >
...
export class RemindersComponent {
  ...
  private nextTriggerTime(r: Reminder): number | null {
    ...
    if (r.type === "occasion" && r.date) {
      ...
      const dd = Number(ddS);
      const year =
        now.getMonth() > mm || (now.getMonth() === mm && now.getDate() > dd)
          ? now.getFullYear() + 1
          : now.getFullYear();
  ...
  add(e: Event) {
    ...
    if (this.type === "occasion")
      r.date = `${String(this.occMonth).padStart(2, "0")}-${String(this.occDay).padStart(2, "0")}`;
  ...
  toggleDone(r: Reminder) {
    this.reminders.update((arr) =>
      arr.map((x) => (x.id === r.id ? { ...x, done: !x.done } : x)),
    );
    this.persist();
  }
